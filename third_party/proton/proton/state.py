...
def enter_state(name: str) -> None:
    ...
    # Proton exposes one native thread-local state. Keep a small Python-side
    # stack so nested metadata helpers can restore the owner metadata state
    # instead of clearing it when an inner state exits.
