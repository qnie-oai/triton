      ...
      for (auto *data : profiler.dataSet) {
        ...
        if (isMetricKernelNode) {
          if (auto metadataOwner = findMetadataOwnerContext(contexts)) {
            ...
            // Keep the metric-copy kernel visible under the metadata state,
            // but attach the copied flexible metric to the compute owner's
            // <metric> child so roofline values still summarize on the kernel.
